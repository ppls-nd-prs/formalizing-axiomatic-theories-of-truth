...
\begin{definition}[syntactic term]\label{def:syntactic-term}
    \uses{def:fol, def:semi-term}
...
Using the concept of semi-terms we then define semi-formulas, from we can derive, by a process comparable to above, the notion of a full syntactic formula.

\begin{definition}[semi-formula]\label{def:sf}
    \uses{def:fol,def:semi-term}
