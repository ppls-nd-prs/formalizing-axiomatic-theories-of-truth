...
We can then work our way up to the level of formulas constructed using our specified languages. As \cite{ffl} want to be able to make a distinction between sentences, i.e. formulas without free variables, and non-sentences, i.e. formulas containing free variables, their formalization of formulas features a disinction between bound and free variables, something which is not present in traditional de Bruijn syntax: there free variables are just those that are indexed by a number higher than the amount of quantifiers ($-1$ as Lean 4 is 0-based). For example in the de Bruijn formula 
\begin{equation}
\forall P(0,1),
\label{eq:db}
\end{equation}
