...
\begin{definition}[semi-term]\label{def:sem-t}
    ...
    \begin{enumerate}
        \item the set of bound variables $BV_n = \{\#i | 0 \leq i \leq n\} \subseteq A_{\mathcal{L},\xi,n}$,
...
We will also need the concept of substitution of terms in terms and formulas with a free bound variable. Traditionally, for example in \cite{negri2001}, substitution is inductively defined over terms, then over formulas. For reasons of integration with Lean 4's preset notions of substitution of terms (Lean 4 is after all a logical system in itself), \cite{ffl} has chosen not to have their substitution method defined over terms, but solely over formulas, making use of Lean 4's innate function for terms. As the inner workings of Lean 4 are outside the scope of this project and on the level of formulas substitution works the same with the traditional definitions of subsitution (for De Bruijn notation) as with the definitions used by \cite{ffl}, we will therefore give the traditional definition of substitution in terms, with the exception that it is denotation will not resemble that of substitution of terms in formulae, accentuating the fact that $t_0/[t_1]$, where $t_0$ and $t_1$ are semiterms, is not defined.

\begin{definition}[substitution in terms]
    Let $\mathcal{A} = A_{\mathcal{L},\mathbb{N},n}$ and $\mathcal{T} = A_{\mathcal{L},\mathbb{N},n}$ be sets of semiterms. Then, the substitution function $\texttt{sub}_t : \mathcal{A} \times \mathcal{T} \to \mathcal{B}$, is recursively defined as
    \begin{enumerate}
        \item if $a \in BV_1$, then $\texttt{sub}_t(a,t) = t$,
        \item if $a \in FV_{\mathbb{N}}$, then $\texttt{sub}_t(a,t) = a$ and
        \item if $a = f(b_1,...,b_k) \in \mathcal{A}$, then $\texttt{sub}_t(a,t) = f(\texttt{sub}_t(b_1,t),...,\texttt{sub}_t(b_k,t))$. 
    \end{enumerate}
\end{definition}

\begin{definition}[substitution]\label{def:subst}
    Let $\mathcal{F} = B_{\mathcal{L},\mathbb{N},n}$ be a set of semiformulae and $\mathcal{T} = A_{\mathcal{L},\mathbb{N},n}$ a set of terms. Then, the substitution function $sub_f : \mathcal{F} \times \mathcal{T} \to \mathcal{F}$, of terms $\textbf{t} \in \mathcal{T}$, where $\textbf{t} = t_1,...,t_n$, in formula $\varphi \in \mathcal{F}$, denoted $\varphi/[\textbf{t}]$, is inductively defined as
    \begin{enumerate}
