...
\begin{definition}[first-order language]\label{def:FO-signature}
    \lean{LO.FirstOrder.Language}
    \leanok
    A language is a structure $((F_i)_{i\in N}, (R_i)_{i\in N})$, where $F_i$ is a set of $i$-ary function symbols and $R_i$ a set of $i$-ary predicate symbols.
\end{definition}

By providing concrete sets of function and relation symbols to $F_i$ and $R_i$ we obtain a specific language.
...
\begin{definition}[$\mathcal{L}_{T}$: the language of peano arithmetic including the truth predicate symbol]
    \uses{def:FO-language}
    \lean{LTr}
    \leanok
