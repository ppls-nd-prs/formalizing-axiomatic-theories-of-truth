...
\begin{definition}[first-order signature]\label{def:FO-signature}
    ...
    \leanok
    A language is a structure $((F_i)_{i\in N}, (R_i)_{i\in N})$, where $F_i$ is a set (/type) of $i$-ary function symbols, etc.


    A first-order language is a structure $\mathcal{S} = (\mathcal{F},\mathcal{R})$ such that:
    \begin{enumerate}
        \item $\mathcal{F}$ is of type \textit{$\mathbb{N} \to \mathcal{S}$} and
        \item $\mathcal{R}$ is of type \textit{$\mathbb{N} \to \mathcal{S}$}.
    \end{enumerate}
\end{definition}
...
\begin{definition}[$\mathcal{L}_{PA}$: the language of peano artihmetic]\label{def:LPA}
    ...
    \lean{LPA}
    \leanok
    $L_PA$ is defined by setting:
 
    $F_0 = {0}$
    $F_1 = {S}$
    $F_2 = {+,x}$
    and $R_2 = { =}$
    The language of Peano arithmetic $\mathcal{L}_{PA}$ is the first-order language $\mathcal{L}_{PA} = (\mathcal{F}, \mathcal{R})$, where
    \begin{enumerate}
        \item 
    \end{enumerate}
\end{definition} 
