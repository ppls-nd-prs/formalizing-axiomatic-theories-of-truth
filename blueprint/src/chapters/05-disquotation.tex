...
\begin{definition}[$L$: List]\label{def:List}
    \lean{List}\leanok
    We define the set of lists with respect to some set $S$, denoted $L(S)$, as the smallest set $X$ such that
    \begin{enumerate}
    \item $[] \in X$
...
\begin{definition}[$\cup$: List Append]\label{def:List-Append}
    \lean{List.union}\leanok
    We define the append function for lists recursively as 
    \begin{enumerate}
    \item $[] \cup L = L$
    \item $[h,T] \cup L = [h,T \cup L]$ 
    \end{enumerate} 
\end{definition}

\begin{definition}[$\varphi L(d)$: The L of Disquoted Formulas wrt a Derivation]\label{def:Rel-Phis}
    \lean{Conservativity.build_relevant_phis_list}\leanok
    \uses{def:List,def:List-Append}
    We define the function $\varphi L(d) : \mathcal{D}(\texttt{TB},\Gamma,\Delta) \to L(\mathcal{F}(\mathcal{L},\{0\}))$ recursively as 
    \begin{enumerate}
    \item $\varphi L(\Gamma \Rightarrow \Delta, A) = \begin{cases}
        ...
        [] & \text{else}
    \end{cases}$
    \item $\varphi L(\frac{d}{\Gamma, \Delta}) = \varphi L(d)$
    \item $\varphi L(\frac{d_1 \quad d_2}{\Gamma \Rightarrow \Delta}) = \varphi L(d_1) \cup \varphi L(d_2)
$
    \end{enumerate}
...
\begin{definition}[$pa\_plus\_to\_pa$: Function Derivations from \texttt{PA} + tau-equivalences to \texttt{PA}]\label{def:PA-Plus-To-PA}
    \lean{Conservativity.FirstOrder.Language.BoundedFormula.FirstOrder.Language.BoundedFormula.pa_plus_to_pa}\leanok
    \uses{def:TB,def:PA,def:FO-Formula}
    
\end{definition}

\begin{definition}[Translation]\label{def:Translation}
    \lean{Conservativity.FirstOrder.Language.BoundedFormula.FirstOrder.Language.BoundedFormula.translation}\leanok
    \uses{def:FO-Formula,def:TB,def:PA}
